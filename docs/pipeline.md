...
- **Калибровочный набор:** строго **500 изображений COCO val2017** (`_CALIBRATION_IMAGE_COUNT = 500`
  ...
  порядке для всех трёх калибраторов **и** для стадии 6: алгоритм калибровки
...
- **Strategy C (Sensitivity Analysis)** — программный поиск N% самых чувствительных слоёв
  (HAWQ-подобный) **пока не реализован** в коде; запланирован на будущее и будет включаться
  явным флагом `--enable-sensitivity-analysis` (по умолчанию выключен), так как профилирование занимает много времени.
