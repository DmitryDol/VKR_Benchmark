...
**Success Criteria** (what must be TRUE):
  1. A single benchmark run produces a per-stage CSV and JSON file containing latency split (pre/infer/post), FPS, jitter, mAP_50, mAP_50:95, IoU, model size, peak VRAM, MACs/FLOPs, and hardware info
  2. Accuracy drop (%) relative to FP32 baseline is computed and logged automatically when baseline results exist
  3. A unified results.csv with a stage column is generated by merging all per-stage files via CLI
  4. User can run `benchmark --model rt-detr --stage 1` to execute one stage and `benchmark --model rt-detr --all-stages` to run all 6 sequentially
