...
- **Побочные продукты:** на этой стадии **один раз** вычисляются MACs/FLOPs (через
  ...
  (решение D-09) — размер входа берётся из `adapter.input_size`, поэтому RF-DETR (704×704) считается
  корректно. Также сохраняется значение mAP@[0.5:0.95], которое становится базой для расчёта
  `accuracy_drop_pct` остальных стадий.
...
- **Откат INT8 → FP16 у RF-DETR:** для трансформерной архитектуры RF-DETR авто-тюнер TensorRT 10.16
  выбирает почти исключительно FP16-ядра даже при флаге INT8 (доля INT8-слоёв ≈ 0–0.78%). Это
  **научно зафиксированный результат**, а не дефект; 5 таких конфигураций RF-DETR (стадии 5 и 6)
  ...
  [`tensorrt_engine.py`](../src/benchmark/engines/tensorrt_engine.py). Подробнее — в
  [models.md](models.md#rf-detr-l-и-эффект-отката-int8).
